...
if(isset($_POST['submit']))
  {
   ...
   $query=mysqli_query($con, "update   bookdarshan set Remark='$remark', Status='$ressta' where ID='$viewid'");
    if ($query) {
...
<body>
 ...
 $ret=mysqli_query($con,"select devotees.ID as uid,devotees.FirstName,devotees.LastName,devotees.MobileNumber,devotees.Email,devotees.StreetName,devotees.Location,devotees.ZipCode,devotees.Landmark,devotees.City,devotees.RegDate,temple.ID as tid,temple.TempleName,temple.TempleLocation,temple.State as tstate,temple.Country as tcountry,temple.Description,temple.TempleImage1,bookdarshan.ID as bid,bookdarshan.BookingNumber,bookdarshan.UserID,bookdarshan.TempleID,bookdarshan.DateofDarshan,bookdarshan.TimeofDarshan,bookdarshan.TotalMember,bookdarshan.Message,bookdarshan.BookingDate,bookdarshan.Status from bookdarshan join devotees on devotees.ID=bookdarshan.UserID join temple on temple.ID=bookdarshan.TempleID where bookdarshan.ID='$viewid';");
